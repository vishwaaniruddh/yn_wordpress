...
<div class="page-container <?php echo esc_attr($extra_class) ?> <?php echo esc_attr($page_column_class['main_class']); ?>">
	...
	// Try different meta keys one by one depending on what you're using
	$image_id = get_term_meta( $term->term_id, 'breadcrumb_background', true );

	// Fallback to category thumbnail if breadcrumb image isn't found
	if ( ! $image_id ) {
	...
	if ( $image_id ) {
		...
		if ( $image_url ) {
			echo '<div class="category-banner-image" style="text-align:center; margin: 20px 0;">';
			echo '<img src="' . esc_url( $image_url ) . '" alt="' . esc_attr( $term->name ) . '" style="max-width: 100%; height: auto; border-radius: 8px;" />';
			echo '</div>';
		}
	}
}
