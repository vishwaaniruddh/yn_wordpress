...
class TS_Shortcodes{
	...
	function add_shortcodes(){
		...
		add_shortcode('ts_coupon', array($this, 'coupon_shortcode'));
		
		add_shortcode('ts_header_notice', array($this, 'header_notice_shortcode'));
	}
	...
	function header_notice_shortcode( $atts ){
		extract($atts);
		
		$icon_text_html = '';
		for( $i = 1 ; $i <= 5 ; $i++ ){
			if( !empty(${'text_' . $i}) ){
				$icon_text_html .= '<div class="item">';
				if( !empty(${'icon_' . $i}) ){
					$icon_text_html .= '<i class="' . esc_attr( ${'icon_' . $i} ) . '"></i>';
				}
				$icon_text_html .= esc_html( ${'text_' . $i} );
				$icon_text_html .= '</div>';
			}
			else{
				break;
		...
		$button_html = '';
		for( $i = 1 ; $i <= 5 ; $i++ ){
			if( !empty(${'button_label_' . $i}) ){
				$button_html .= '<a class="button-text" href="' . esc_url( isset(${'button_url_' . $i}) ? ${'button_url_' . $i} : '' ) . '">' . esc_html( ${'button_label_' . $i} ) . '</a>';
			}
			else{
				break;
			}
		...
		if( !$icon_text_html && !$button_html ){
			return;
		}
		
		ob_start();
		?>
		<div class="ts-header-notice-shortcode">
		<?php
		echo $icon_text_html;
