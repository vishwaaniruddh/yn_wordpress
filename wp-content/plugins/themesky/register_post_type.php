...
if( !class_exists('TS_Team_Members') ){
	class TS_Team_Members{
		...
		function __construct(){
			...
			$this->thumb_size_array = array(340, 0);
			$this->add_image_size();
	...
	class TS_Logos{
		...
		function setting_page_content(){
			$options_default = array(
							...
							,'responsive' => array(
								'break_point'	=> array(0, 300, 550, 700, 950, 1200, 1400, 1600, 1800)
								,'item'			=> array(2, 3, 4, 5, 6, 7, 8, 9, 10)
							)
						);
...
/*** TS Custom Blocks ***/
if( !class_exists('TS_Custom_Blocks') ){
	class TS_Custom_Blocks{
		public $post_type;
		
		function __construct(){
			$this->post_type = 'ts_custom_block';
			add_action('init', array($this, 'register_post_type'));
		}
		
		function register_post_type(){
			$labels = array(
				'name' 				=> esc_html_x( 'Custom Blocks', 'post type general name', 'themesky' ),
				'singular_name' 	=> esc_html_x( 'Custom Block', 'post type singular name', 'themesky' ),
				'add_new' 			=> esc_html_x( 'Add New', 'custom block', 'themesky' ),
				'add_new_item' 		=> esc_html__( 'Add New', 'themesky' ),
				'edit_item' 		=> esc_html__( 'Edit Custom Block', 'themesky' ),
				'new_item' 			=> esc_html__( 'New Custom Block', 'themesky' ),
				'all_items' 		=> esc_html__( 'All Custom Blocks', 'themesky' ),
				'view_item' 		=> esc_html__( 'View Custom Block', 'themesky' ),
				'search_items' 		=> esc_html__( 'Search Custom Block', 'themesky' ),
				'not_found' 		=> esc_html__( 'No Custom Blocks Found', 'themesky' ),
				'not_found_in_trash'=> esc_html__( 'No Custom Blocks Found In Trash', 'themesky' ),
				'parent_item_colon' => '',
				'menu_name' 		=> esc_html__( 'Custom Blocks', 'themesky' )
			);
			$args = array(
				'labels' 			=> $labels,
				'public' 			=> true,
