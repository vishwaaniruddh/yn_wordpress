...
class TS_Elementor_Addons{
				...
				if( $query_name != 'product_review' ){
					...
					if( is_array($reviews) && !empty($reviews) ){
						...
						foreach( $reviews as $review ){
							$results['ids'][] 	= $review->comment_ID;
							$results['names'][] = $this->autocomplete_format_name( $review->comment_content, $review->comment_ID, 10 );
						}
						die( json_encode($results) );
					}
				}
		...
		if( isset( $_GET['search_term'] ) ){
			...
			if( $query_type == 'post' ){
				if( $query_name != 'product_review' ){
					...
					if( is_array($reviews) && !empty($reviews) ){
						foreach( $reviews as $review ){
							$results[] 	= array( 'id' => $review->comment_ID, 'text' => $this->autocomplete_format_name( $review->comment_content, $review->comment_ID, 10 ) );
						}
					}
				}
			}
	...
	function autocomplete_format_name( $name, $id, $limit_words = 0 ){
		if( $limit_words ){
			$name = wp_trim_words( $name, $limit_words, '...' );
		}
