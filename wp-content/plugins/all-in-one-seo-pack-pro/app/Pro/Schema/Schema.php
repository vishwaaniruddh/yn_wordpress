...
$graphs = aioseo()->schema->generatingValidatorOutput || ! empty( $graphs )
	? $graphs
	: ( isset( $metaData->schema ) && isset( $metaData->schema->graphs ) ? $metaData->schema->graphs : [] );

$userDefinedGraphs = [];

if ( is_array( $graphs ) || is_object( $graphs ) ) {
	foreach ( $graphs as $graphData ) {
		...
		// Replace graph name with subtype if available
		...
		switch ( $graphData->graphName ) {
			case 'FAQPage':
				...
				$this->faqPages         = array_merge(
					$this->faqPages,
					$this->faqPageInstance->get( $graphData )
				);
				break;
...
$customGraphs = aioseo()->schema->generatingValidatorOutput || ! empty( $customGraphs )
	? $customGraphs
	: ( isset( $metaData->schema ) && isset( $metaData->schema->customGraphs ) ? $metaData->schema->customGraphs : [] );
...
$default = aioseo()->schema->generatingValidatorOutput || ! empty( $default )
	? $default
	: ( isset( $metaData->schema ) && isset( $metaData->schema->default ) ? $metaData->schema->default : [] );
